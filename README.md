...
У `ZapretManager` есть не только тёмная, но и полноценная светлая тема. Все основные сценарии, таблицы и окна поддерживают режимы `Светлая`, `Тёмная` и `Как в системе`.
...
Во вкладке `Telegram / Proxy` собраны настройка `MTProto`, управление `TG WS Proxy`, статус автозапуска, `CF-proxy`, тест доступности и работа с датацентрами Telegram.
